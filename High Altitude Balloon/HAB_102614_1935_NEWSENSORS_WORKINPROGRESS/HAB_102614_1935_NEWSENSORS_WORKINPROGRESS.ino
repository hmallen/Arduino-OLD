...
// Accelerometer
const int accelXPin = A7;
const int accelYPin = A8;
const int accelZPin = A9;
...
const int cfcPin = A6;  // Pin for reading CFC levels from sensor
const int metPin = A7;  // Pin for reading methane levels from sensor
...
void setup() {
  ...
  Wire.begin();
  Serial.println(F("NOODLEZLOL"));
  configureSensors();
  Serial.println(F("NOODLEZLOL1"));
  baro.init();
  Serial.println(F("NOODLEZLOL2"));
  readAltimeter();
  Serial.println(F("NOODLEZLOL3"));
  while(altCm >= 152) {
    zeroAltimeter();
    altZeroedPrev = true;
    delay(2000);
  ...
  Serial.println(F("NOODLEZLOL4"));
  zeroSensors();
  Serial.println(F("NOODLEZLOL5"));
  gpsGetData();
  syncRequired = false;
  checkAltCalib();
#ifdef debugMode
...
void zeroSensors() {
  ...
  int gyroXTot, gyroYTot, gyroZTot;
  // Accelerometer
...
void configureSensors() {
  // Gyroscope
  i2cWriteByte(gyroAddr, GYRO_CTRL_REG1, 0x1F);    // Turn on all axes, disable power down
  i2cWriteByte(gyroAddr, GYRO_CTRL_REG3, 0x08);    // Enable control ready signal
  i2cWriteByte(gyroAddr, GYRO_CTRL_REG4, 0x80);    // Set scale (500 deg/sec)
  // Compass
