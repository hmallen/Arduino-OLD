...
void setup() {
  ...
  delay(10);
  Serial3.end();			// clear all rx buffer
  delay(5);
  Serial3.begin(38400);
  rx_ready = false;
#endif
...
void VC0706_osd_add_char(int col, int row, String osd_string) {
  ...
  if(string_length > 14) string_length = 14;  // max 14 osd characters
  ...
  for(i = 0; i < string_length; i++) tx_vcbuffer[i + 6] = osd_string.charAt(i);
 ...
 * Description	  : Transmit buffer to Arduino Serial3 Monitor
 *					
 * Input		  : tx_vcbuffer
 *					
 * Output		  : None
...
void debug_send() {
  ...
  for(i = 0; i < tx_counter; i++) {
    Serial3.print(tx_vcbuffer[i], HEX);
    Serial3.print(", ");
  }
...
void buffer_send() {
  ...
  for(i = 0; i < tx_counter; i++) Serial3.write(tx_vcbuffer[i]);
...
void buffer_read() {
  ...
  if(rx_ready) return;  // if something unread in buffer, just quit

  rx_counter = 0;
  VC0706_rx_buffer[0] = 0;
  while(Serial3.available() > 0) {
    VC0706_rx_buffer[rx_counter++] = Serial3.read();
    //delay(1);
  }

  if(VC0706_rx_buffer[0] != 0x76) validity = false;
  if(VC0706_rx_buffer[1] != VC0706_SERIAL_NUMBER) validity = false;
