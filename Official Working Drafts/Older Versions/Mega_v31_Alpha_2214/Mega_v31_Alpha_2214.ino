...
void loop() {
  ...
  doorStateRead();  // Check door (open/closed) status and relay information, if necessary
  ...
  h2oAmtCons = h2oLastLevel - h2oLevel;
  h2oAmtConsTot += h2oAmtCons;
  foodAmtCons = foodLastLevel - foodLevel;
  foodAmtConsTot += foodAmtCons;
  ...
  foodCheckChange();  // Check for food level change, low level, or empty bucket before next loop
...
void doorStateRead() {
  ...
  if(doorState == 1 && doorLastState == 0) {  // If door is detected as open and was closed on last read
    ...
    if(doorTimer == 0 || doorLastTimer > doorRefillInterval) {  // If door timer never started or refill interval exceeded
...
void h2oCheckChange() {
  if(h2oEmpty == false) {  // If bucket isn't empty according to boolean value currently stored,**
    if(h2oLevel == 0) {  // **but h2o level has just been detected at 0
      Serial.println("H2O bucket empty or currently being refilled.");
      ...
      if(h2oAmtCons >= h2oChangeTrigger) {  // If h2o level has changed significantly since last read
        Serial.print(">= 10% change in h2o detected. Confirming...");
...
void waitH2ORefill() {
  ...
  while(h2oEmpty == true) {  // As long as boolean continues to indicate that bucket is empty
    ...
    if(h2oLevel >= h2oFullLevel) {  // If h2o level is now above the threshold to be considered "refilled" or "full"
      ...
      Serial.println("Waiting for h2o bucket to be refilled.");  // Present some messages to the user
...
void foodCheckChange() {
  if(foodEmpty == false) {  // If bucket isn't empty according to boolean value currently stored,**
    if(foodLevel == 0) {  // **but food level has just been detected at 0
      Serial.println("Food bucket empty or currently being refilled.");
      ...
      if(foodAmtCons >= foodChangeTrigger) {  // If food level has changed significantly since last read
        Serial.print(">= 10% change in food detected. Confirming...");
        delay(5000);  // Pause briefly to let conditions settle in case bucket was bumped, etc.
...
void errorCheck() {
  if(openCount == 0 && h2oAmtCons < h2oErrorTrigger) {  // If door hasn't been opened, an increase in h2o level detected,**
    if(h2oEmpty == true) {  // **and bucket is currently empty
      errorCode = 2;  // Set corresponding error code
    }
    else if(h2oEmpty == false) {  // **and bucket is not currently empty
...
/*void defineEvent() {
 ...
 break;
 case 2:
 errorString += "Bucket empty and H2O level increase detected while stall door closed.";
 break;
 }
 }*/
