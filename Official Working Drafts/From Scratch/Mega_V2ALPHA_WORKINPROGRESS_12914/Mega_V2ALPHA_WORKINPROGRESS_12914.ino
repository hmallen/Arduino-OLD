/*
 ...
 Error codes:
 0 = None
 1 = Increase in h2oLevel while stall door closed
...
const unsigned long doorErrInterval = 2 * 60 * 1000;  // Door error timeout [1st digit minutes] in milliseconds
...
// Errors and undefined placeholders
int errorCode = 0;  // Integer representing specific error (codes listed above, at bottom of introductory comment)
String errorString;  // Place-holder for error string to be written to SD when necessary
...
void defineError() {
  ...
  case 0:
    errorString += "Improper function call. No error detected (errorCode = 0).");
    return;
  case 1:
...
void h2oLevelRead() {
  ...
  h2oLevel = map(h2oReadVal, 0, 921, 0, 100);  // Set h2oLevel to 0-100 scale by mapping analogRead value
...
// Separated into separate function for future use when error checks required
void doorCheck() {
  if(doorTimer < doorErrTimer && amtCons < h2oErrorTrigger) {
  }
}
