...
//#include <"NAME_OF_RFID_LIBRARY".h>
#include <SdFat.h>
...
#define gprsSlaveSelect 10  // SPI chip select pin (SS) for SD card (specific to Arduino board)

#define statusLED 2  // Pin for LED to indicate ready status

smsTargetNum[9] = 2145635266;  // Mobile number to send data via SMS
...
void setup() {
  ...
  if(!sd.begin(gprsSlaveSelect, SPI_FULL_SPEED)) {
    sd.initErrorHalt();
  }
#ifdef debugMode
