...
int doorLastState = 0;  // Remembers door state on last read so openCount only incremented when door closed first
...
void errorCheck() {
  if(openCount == 0 && amtCons < h2oErrorTrigger) {  // If door hasn't been opened, an increase in h2o level detected,**
