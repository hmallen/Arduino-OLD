/*
 ...
 - Possible errors in data aggregation
 - Switch void functions to defined data type for return to increase efficiency
...
int currTimer = 0;  // Reset to millis() for current time on each loop
...
const int h2oErrorTrigger = -5;  // % positive change without preceeding door open even that triggers error write to SD
...
void setup() {
  ...
  lcd.begin(16, 2);  // Initiates LCD display connection with 16 x 2 dimensions
...
void sdWriteError() {
  ...
  if(dataFile) {  // If file on SD card is opened successfully
    ...
    Serial.println("Error opening file for writing error.");
  }
}
