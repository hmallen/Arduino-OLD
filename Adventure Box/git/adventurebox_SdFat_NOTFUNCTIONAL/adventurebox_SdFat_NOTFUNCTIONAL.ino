...
void gpsdump(TinyGPS &gps) {
  ...
  if (pathSetup == true) {
    pathSetup = false;
    String directoryNameRaw = monthFormatted + dayFormatted + String(year) + "_" + String(tripNumber);
    for (int x = 0; x < 15; x++) {
      char c = directoryNameRaw.charAt(x);
      directoryName[x] = c;
    }
  }
...
void createPaths() {
  ...
  if (!sd.mkdir(directoryName)) sd.errorHalt("Failed to create directory.");
  if (!logFile.open(logFileName, O_CREAT | O_WRITE)) sd.errorHalt("Failed to create log file.");
  logFile.close();
  if (!infoFile.open(infoFileName, O_CREAT | O_WRITE)) sd.errorHalt("Failed to create info file.");
  infoFile.close();
}
