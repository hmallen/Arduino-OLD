...
void setup() {
  ...
  for(; (millis() - gpsAltCalibDelay) < gpsAltCalibTimeout;) {
    ...
    int waitTime = (gpsAltCalibTimeout - (millis() - gpsAltCalibDelay)) / 60000;
...
void gpsdump(TinyGPS &gps) {
  ...
  if(gpsLock == true) {
    ...
    // Time String
    byte hourCorrected;
    int hourOffset = hour + gpsGmtOffsetHours;
    if(hourOffset > 0) hourCorrected = hour;
    else if(hourOffset < 0) hourCorrected = 24 - abs(hourOffset);
    else hourCorrected = 0;
...
void gprsGetDateTime() {
  ...
  Serial2.println(F("AT+CCLK?"));  // Read date/time from GPRS
