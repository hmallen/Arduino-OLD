...
#define debugModeSwitch  // If defined, asks for serial input to control servo position and print sensor values (debugMode must also be defined)
...
/*Controls rate of rotation when travelling over the full range while using slow servo functions.
...
#endif
#ifdef debugMode
#ifdef debugModeSwitch
  Serial.println(F("Make sure monitor is set to \"Both NL & CR\" before continuing."));
  Serial.println(F("Type the following characters to control unit:"));
  Serial.println(F("\"n\" = Minimum"));
  Serial.println(F("\"m\" = Middle"));
  Serial.println(F("\"x\" = Maximum"));
  Serial.println(F("\"v\" = Print sensor values"));
  Serial.println();
#endif
#endif
}
