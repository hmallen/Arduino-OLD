...
public:
    ...
    int32_t altGetAlt(void)
    {
      return AcquireAveragedSampleCm(NUM_SAMP_FOR_AVG);
    }

    int32_t altGetTemp(void)
    {
      return AcquireAveragedSampleTemp(NUM_SAMP_FOR_AVG);
    }

    int32_t altGetPress(void)
    {
      return AcquireAveragedSamplePress(NUM_SAMP_FOR_AVG);
    }
...
protected:
    ...
    virtual int32_t AcquireAveragedSamplePress(const uint8_t nSamples) = 0;
    ...
    int32_t PascalToCentimeter(const int32_t pressurePa)
    {
      ...
      static const int32_t PA_INIT       = 108609;  // Baseline corrected from Wunderground data
...
public:
    ...
    BaroPressure_MS5607B(bool CSB = false) : 
    i2cAddr_((CSB ? 0xEC : 0xEE) >> 1) {
    }
...
private:
    ...
    virtual int32_t AcquireAveragedSampleTemp(const uint8_t nSamples)
    {
      ...
      const int32_t dT = tempAvg - coefficients_[4] * 256;  // difference between actual and reference temperature
      const int32_t temp  = (2000 + (dT * coefficients_[5]) / pow(2, 23));  // actual temperature

      return temp;
    }

    virtual int32_t AcquireAveragedSamplePress(const uint8_t nSamples)
    {
      int64_t pressAccum = 0;
