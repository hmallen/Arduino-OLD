...
void setup() {
  ...
  Serial3.begin(19200);  // Hardware serial for GPS communication
  ...
  if(gpsLandDetect == false || altLandDetect == false) {
    ...
    if(finalDataSent == true) {
      ...
      while(finalDataSent == true) {
        ...
        while(millis() - lastSent < menuSendDelay) {
          gprsSMSReceive();
          delay(1);
        }
        gprsSMSMenu();
  ...
  if(gpsLock == true) {
    ...
    if(syncRequired == true) {
      ...
      if(flat != 0 && flon != 0) {
        ...
        // Code placed in here will execute if GPS coordinates are "0" immediately following final read
      }
      if(finalDataSent == false) {
...
void gprsSMSMenu() {
  ...
  Serial2.println(F("\""));
  delay(100);
  Serial2.println(F("Reply with desired option #: 1 - GPS / 2 - Buzzer / 3 - Smoke"));
  delay(100);
  Serial2.println((char)26);
  delay(1000);
  Serial2.flush();
...
#ifdef debugMode
      Serial.println(F("Sender's number of SMS does not match expected. Aborting."));
#endif  
    }
