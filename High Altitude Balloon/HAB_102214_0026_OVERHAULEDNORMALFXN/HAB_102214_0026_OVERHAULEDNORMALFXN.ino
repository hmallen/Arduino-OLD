...
void setup() {
  ...
  //if(!sdCreateLogFile()) sd.errorHalt("Failed to create directory and/or log file.");
...
// SD Card log file and parent directory
boolean sdCreateLogFile() {
  ...
  if(!sd.exists("/logs/")) if(!sd.mkdir("/logs")) return false;  // Couldn't create logs directory
  if (!sd.chdir("/logs")) return false;  // Failed

  time_t t = now();
  int yr = year(t) - 2000;
  sprintf(logFile,"%.2d%.2d%.2d00.csv", yr, month(t), day(t));

  for(int i = 0; i < 100; i++) {
    logFile[6] = i/10 + '0';
    logFile[7] = i%10 + '0';
    if(sd.exists(logFile)) continue;
    if(dataFile.open(logFile, O_RDWR | O_CREAT | O_AT_END)) return true;  // Success
