/*
 ...
 - Add consequences for h2oLevel > h2oLastLevel under various conditions
...
void h2oCheckChange() {
  if(h2oEmpty == false) {  // If bucket isn't empty according to boolean value currently stored,**
    if(h2oLevel == 0) {  // **but h2o level has just been detected at 0
      Serial.println(F("H2O bucket empty or currently being refilled."));
...
void waitH2ORefill() {
  ...
  while(h2oEmpty == true) {  // As long as boolean continues to indicate that bucket is empty
    ...
    if(h2oLevel >= h2oFullLevel) {  // If h2o level is now above the threshold to be considered "refilled" or "full"
      ...
      Serial.println(F("Waiting for h2o bucket to be refilled."));  // Present some messages to the user
