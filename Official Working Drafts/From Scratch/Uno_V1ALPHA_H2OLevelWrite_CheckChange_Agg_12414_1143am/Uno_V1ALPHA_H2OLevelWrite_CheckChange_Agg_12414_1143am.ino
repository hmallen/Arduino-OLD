...
Prototype v1.0 ALPHA
 ...
 THIS SKETCH WAS ADAPTED FROM A SKETCH WRITTEN FOR THE ARDUINO MEGA, SO ERRORS BEYOND THOSE LISTED MAY EXIST
 
 Board: Arduino Uno R3+
 - This program is specific to the Arduino Uno. 2K SRAM is insufficient to accomodate the serial prints in the 
 program, so flash assist macros [F()] are included in the sketch. With only 1 hardware serial available on the 
 Uno, the GPRS jumpers are set for software serial transmission, and the connection is initiated in the sketch 
 through "gprsSerial" via SoftwareSerial.
...
#define rxPin 7  // Defines pin for GPRS software serial data transmission
#define txPin 8  // Defines pin for GPRS software serial data transmission
#define powPin 9  // Defines pin for software power-up of GPRS shield
#define chipSelect 10  // Defines SPI chip select pin (SS) for SD card (specific to Arduino board)
...
void loop() {
  ...
  if(lapTimer >= aggInterval) {  // If aggregation timer has exceeded aggregation interval
    ...
    Serial.println(F("Aggregate data written to SD card."));
...
void time12hrConst() {
  ...
  if(hour.toInt() > 12) {  // If integer format of 24hr formatted hour is more than 12
    ...
    time12hrString += "12";  // Add new 12hr formatted hour to 12hr time string
    time12hrString += ":";  // Add colon to create properly formatted time
    time12hrString += String(minute);  // Add minute string after the colon
    ...
    time12hrString += String(hour.toInt());  // Add new 12hr formatted hour to 12hr time string
    time12hrString += ":";  // Add colon to create properly formatted time
...
void h2oLevelRead() {
  int h2oReadVal = analogRead(A0);  // Read value from sensor
...
void h2oCheckChange() {
  ...
  if(h2oEmpty == false) {  // If bucket isn't empty according to boolean value currently stored,
    if(h2oLevel == 0) {  // but h2o level has just been detected at 0
      ...
      if(abs(h2oLevel - h2oLastLevel) >= h2oChangeTrigger) {  // If h2o level has changed significantly since last read
        ...
        if(abs(h2oLevel - h2oLastLevel) >= h2oChangeTrigger) {  // If h2o change detected was real
          ...
          h2oLastLevel = h2oLevel;  // Set h2o level comparison value equal to current h2o level
        }
        else {  // If, after 2nd read, there wasn't actually a change
          Serial.println(F("detected change was a false-positive."));  // Tell user that the detection was a false-positive
          Serial.println();
        }
      }
    }
    ...
    h2oLastLevel = h2oLevel;  // Set h2o level storage value equal to current h2o level
  }
}
...
void sdWriteData() {
  ...
  if(dataFile) {  // If file on SD card is opened successfully
    ...
    Serial.println(F("Error opening file while attempting to write to SD."));
  }
}

void aggregateWriteSD() {
  Serial.print(F("Units consumed since last data aggregation: "));
