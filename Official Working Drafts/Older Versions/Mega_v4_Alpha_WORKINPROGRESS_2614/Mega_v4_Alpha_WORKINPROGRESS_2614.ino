...
void calibAnalogSensors() {
  ...
  uint16_t h2oCalibMin = analogRead(h2oPin);
  uint16_t foodCalibMin = analogRead(foodPin);
  ...
  uint16_t h2oCalibMax = analogRead(h2oPin);
  uint16_t foodCalibMax = analogRead(foodPin);
  Serial.println(F("Release the button to continue."));
  ...
  Serial.println("analogRead values:");
  ...
  Serial.println(h2oMinRemain);
  Serial.println(h2oMaxRemain);
  Serial.println(foodMinRemain);
  Serial.println(foodMaxRemain);
...
void retrieveAnalogMinMax() {
  ...
  maxFoodVal = foodMaxA + foodMaxB + foodMaxC + foodMaxD;
}
