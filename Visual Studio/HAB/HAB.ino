 ...
 ***********************************************************************************************************
 ...
 - Keep gas sensors away from Sensiron temp & humidity sensor to prevent abnormally high temp reads        *
 ***********************************************************************************************************
 
 To Do:
 - Add protoshield LED for debugMode switch trigger
 - FIX GPSCHECKCHANGE FUNCTION!!!!!!!!!!!!!!!!
 - Add SMS of coordinates just prior to landing in case of device damage (ex. impact, water, etc.)
 ...
 - It would be TOTALLY F'ING AWESOME if it would update course_to/cardinal and distance_to from mobile coord info
...
const int gpsGmtOffsetHours = -6;  // Offset in hours of current timezone from GMT (CST: -5 DST / -6 [No DST])
...
float altFt, altFtPrev, altVertRateFt, altPressMillibar;
...
// Temperature & humidity (Sensiron)
float sensironTempC;  // Retrieves temperature in degrees celsius
float sensironHumidity;  // Retrieves relative humidity in percent
...
void loop() {
  ...
  if(altLandDetect == false) {
    ...
    sensironTempC = sht1x.readTemperatureC();
    sensironHumidity = sht1x.readHumidity();
    // Gas sensors
...
void sdWriteData() {
  ...
  dataFile.print(F("$SEN@"));
  dataFile.print(sensironTempC);
  dataFile.print(F(","));
  dataFile.print(sensironHumidity);
  ...
  if(altLandDetect == true && finalDataSent == false) {
...
void serialWriteData() {
  ...
  Serial.print(F("$SEN@"));
  Serial.print(sensironTempC);
  Serial.print(F(","));
  Serial.print(sensironHumidity);
  Serial.print(F("$ACC@"));
...
void altCheckChange() {
	...
	const float altVertRateFtThreshold = 1.0;  // Difference in consecutive altimeter reads (cm) to qualify as a real change in altitude

	if (startupComplete == true && altLandDetect == false) {
		if (altAscentDetect == false) {
			if (altAscentDetect == false){
				if (altVertRateFt > altVertRateFtThreshold) altPosRead++;
				else if (altVertRateFt < altVertRateFtThreshold) altPosRead = 0;  // Latest read doesn't carry same sign as previous, so reset positive read counter
				...
				if (altVertRateFt < -altVertRateFtThreshold) altNegRead++;
				else if (altVertRateFt > -altVertRateFtThreshold) altNegRead = 0;  // Latest read doesn't carry same sign as previous, so reset negative read counter
				else;  // No change in altitude detected between reads, so do nothing
		...
		else if (altAscentDetect == true && altDescentDetect == true) {	// CHECK ALL OF THIS CODE BELOW...NO IDEA WHAT IS GOING ON !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			int altCmChangeThreshold = 30;
...
void gpsdump(TinyGPS &gps) {
  ...
  if(gpsLock == true) {
    ...
    else if(altLandDetect == false) {  // MAKE SURE THIS IS ACTUALLY THE CONDITION YOU WANT TO TRIGGER THIS OUTPUT BEFORE LIVE LAUNCH!!!!!!!!!!!!!!!!!
      gps.f_get_position(&flat, &flon, &age);
      gpsLat = flat;
      gpsLon = flon;
      altitude = gps.f_altitude();
...
void gprsSMSConfirmation(int menuVal) {
	switch(menuVal) {
	case 1:
		Serial2.println(F("AT+CMGF=1"));
		delay(100);
...
void executeSMSCommand() {
  ...
  case 2:
	...
	gprsSMSConfirmation(1);
    break;
  case 3:
	// Trigger warning just prior to smoke signal ignition
	  digitalWrite(piezoPin, HIGH);
	  delay(5000);
	  digitalWrite(piezoPin, LOW);
