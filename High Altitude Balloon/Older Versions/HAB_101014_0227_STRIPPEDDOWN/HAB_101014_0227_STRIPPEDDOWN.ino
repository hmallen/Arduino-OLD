...
#ifdef debugMode
    if(startupComplete == true) Serial.println(altPosRead);
...
#ifdef debugMode
    if(startupComplete == true) Serial.println(altNegRead);
...
void readAccelerometer() {
  ...
  if(ReadByte(accelAddr, 0x0, &devId) != 0) Serial.println("Cannot read device ID from sensor");
    ...
    Serial.print(devId);
    Serial.println(" expected 0xE5 (ACCELEROMETER)");
  }
  else {
    // Read the output
    ...
    else Serial.println("Failed to read from accelerometer.");
  }
}
...
void readCompass() {
  ...
  else Serial.println(F("Failed to read from compass."));
}
...
void readGyroscope() {
  ...
  else Serial.println("Failed to read from gyroscope.");
}
