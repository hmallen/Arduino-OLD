/*FIX '+' IN SERIAL OUTPUT OF FINAL GPS COORDINATES
FIX SECOND GPS READ EVENT
CHECK IF IT'S NECESSARY TO END GPRSSERIAL BETWEEN FINAL GPS DATA ACQUISITION EVENTS
ADD LED EVENT TO SIGNAL UNIT IS READY TO BEGIN RECORDING
ADD METHOD FOR ACQUIRING MULTIPLE EVENTS WITH DIFFERENT FILE NAMES OR SEPARATOR
-- ADD DATE/TIME TAG TO BEGINNING OF EACH RECORDING
ADD POINT ON MAP TO INDICATE LAUNCH SITE*/

//#define debugMode  // Comment out this line to write to SD instead of serial monitor
...
void loop() {
  recCheck();
  if(recState == true && digitalRead(landPin) == LOW) {
  ...
  if(recState == true && transmitState == true) {
    // First GPS data acquisition attempt
    ...
    gprsSerial.end();
    delay(1000);
    // Second GPS data acquisition in case of failed first attempt
    if(digitalRead(readyPin) == LOW) {
      while(digitalRead(readyPin) == LOW) {
        delay(1);
      }
