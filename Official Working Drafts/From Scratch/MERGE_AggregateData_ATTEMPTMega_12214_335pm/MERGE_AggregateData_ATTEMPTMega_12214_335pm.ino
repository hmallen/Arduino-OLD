...
unsigned long aggInterval = 15000;
...
void loop() {
  ...
  if (lapTimer >= aggInterval) {
    h2oLevelRead();
    Serial.println(F("Data aggregation occurs here."));
    Serial.println();
    //h2oAmtConsSince();
    //h2oWriteSD();
    prevTimer = millis();
  ...
  else if(intervalTimer >= interval) {
    h2oLevelRead();
    h2oWriteSD();
    prevTimer = millis();
  }
    ...
    if(abs(h2oLevel - h2oLastLevel) <= h2oChangeTrigger) {
      ...
      Serial.print(F(">5% change detected. Confirming..."));
      delay(5000);
      h2oLevelRead();
      if(abs(h2oLevel - h2oLastLevel) >= h2oChangeTrigger) {
        ...
        Serial.println(F("detected change was a false-positive."));
        Serial.println();
      }
    }
  }
...
void sdWriteData() {
  ...
  if(dataFile) {  // If file on SD card is opened successfully
    ...
    Serial.println(F("Error opening file while attempting to write to SD."));  // If data file can't be opened, produce an error message
  }
}
