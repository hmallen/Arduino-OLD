...
void h2oLevelRead() {
  ...
  h2oLevel = map(h2oReadVal, minH2OVal, maxH2OVal, 0, 100);  // Set h2oLevel to 0 - 100 scale by mapping analogRead value
...
void foodLevelRead() {
  ...
  foodLevel = map(foodReadVal, minFoodVal, maxFoodVal, 0, 100);  // Set h2oLevel to 0 - 100 scale by mapping analogRead value
...
void dataWriteSD() {
  ...
  if(dataFile.open("LOGDAD.TXT", O_RDWR | O_CREAT | O_AT_END)) {
